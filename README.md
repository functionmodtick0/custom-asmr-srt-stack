...
2026-07-01 all8 front120 batch CLI 평가에서 official Qwen3-ASR 1.7B local snapshot은 practical CER 59.7%, time-aligned 500ms 16.0%, review effort 100%로 실패했습니다. Granite base보다 text CER는 낮지만 product gate에는 한참 못 미칩니다. Energy VAD의 과거 combined-RMS coverage는 실제 channel waveform과 달랐습니다. Runtime-parity stereo 기본 VAD recall은 `77.35%`입니다. Near-continuous ASMR용 20초 비중첩 challenger는 recall `100%`, precision `87.28%`이며 Bro stereo all8 practical CER `64.33% -> 57.70%`, channel-aware CER `48.21% -> 46.72%`, timing 500ms `15.85% -> 21.95%`, unbiased channel accuracy `52.44% -> 54.88%`를 모두 개선해 automated VAD/chunking gate를 통과했습니다. 같은 all8 set에서 `neosophie/Qwen3-ASR-1.7B-JA` local snapshot은 practical CER 59.4%, time-aligned 500ms 16.0%, review effort 100%로 Qwen official보다 text만 아주 조금 낫지만 product gate를 통과하지 못했습니다. Qwen3-ForcedAligner context 500/2000ms 실험도 Qwen official all8의 time-aligned 500ms를 각각 11.1%/6.9%로 낮춰 기본값으로 승격하지 않습니다. 같은 aligner를 reference-copy oracle에 적용했을 때도 time-aligned 500ms가 95.1%에서 51.2%로 떨어졌으므로, 현재 Qwen3-ForcedAligner는 기본 alignment 계층으로 승격하지 않습니다. 현 최선 alignment 정책은 no-op baseline이지만 reference-copy proxy pass일 뿐입니다. Continuous20 candidate의 channel energy match는 `49.28%`로 85% gate보다 낮고, reference L/R label도 energy와 30개 mismatch/18개 uncertain이므로 channel attribution은 완료되지 않았습니다. 최신 ASR-only blocker는 reference와 channel attribution이며 text ASR도 별도 product quality blocker입니다.
...
uv run casrt vad coverage-cases cases/case-index.json --channel-mode stereo --json -o vad-coverage-stereo.json
...
현재 사용자 실데이터에서 먼저 시험할 development-leading 후보는 exact native HF snapshot `jaykwok/Qwen3-ASR-1.7B-JA-Anime-Galgame-hf@5a6a789ceb2f22d2b8606743b13a8159af218362`입니다. 01/04/08 spot-audit에서 04/08의 도메인 어휘와 ASMR 표현은 Bro보다 좋아졌지만 01의 한자 오인과 L/R bleed duplicate/후반 누락은 남았습니다. 제품이 자동 선택하지 않으므로 모델 modal/CLI에 local snapshot path를 직접 넣습니다.
...
현재 durable human-review 시작점은 `.casrt/experiments/all8-front120-review-cases/case-index.json`입니다. `/home/brain-offloaded/vscode/asmr/whisperx-webui/data/uploads` 01~08 WAV와 2025-12-22 stable-ts draft SRT에서 만든 120초 pseudo-gold set이며, 82개 reference segment 모두 아직 명시적 `content_reviewed` 증거가 없습니다. Bro stereo 후보 350개 segment는 canonical case set의 candidate draft로 연결되어 있고 reference digest는 연결 전후 동일합니다. Continuous20 95-segment challenger는 corrected evaluator에서 VAD downstream gate를 통과했지만 reference/channel authority가 없으므로 canonical에 붙이지 않고, human-reviewed 재평가용 `.casrt/experiments/all8-front120-bro-stereo-continuous20-attach-plan.json`만 보존합니다. 권장 WebUI Review path는 `.casrt/experiments/all8-front120-combined-content-channel-human-reference-review-pack`입니다. 이 pack 하나에 전체 content 미검수 82개, 자동 flag 15개, channel mismatch 30개와 uncertain 18개를 중복 없이 82개 full-segment clip으로 담았습니다. 기존 `검수 완료` 버튼으로 content를 검수하고 channel evidence가 있는 항목은 같은 화면에서 channel도 판정한 뒤 `freeze-case-references --reference-type human-reviewed`로 승격합니다. 현재 corrected readiness 기준 ASR-only blocker는 reference와 channel attribution이고, product quality blocker에는 text ASR도 추가됩니다. Alignment의 no-op pass는 reference-copy proxy 판정일 뿐이며 human-reviewed 기준으로 재평가하기 전에는 최종 완료 판정이 아닙니다.
...
`audit-review-case-channels`는 prepared reference의 L/R label을 stereo energy와 비교합니다. Output `custom-asmr-reference-channel-audit-suite-v1`은 segment id/time/channel, L/R dBFS, energy channel, match/mismatch/uncertain status만 저장하고 transcript text는 저장하지 않습니다. Raw mismatch/uncertain count는 진단 수치로 남고, master segment에서 사람이 `channel_reviewed=true`로 확인한 예외를 뺀 `unresolved_*` count만 review queue와 gate를 막습니다. `--review-effort-output`은 unresolved mismatch/uncertain segment를 기존 `review-pack`에 넣을 수 있는 queue로 만듭니다. 긴 segment는 원본 `start_ms/end_ms`를 보존하면서 최대 5초 `review_clip_start_ms/review_clip_end_ms` 증거 구간을 같이 저장해 사람이 channel evidence만 빠르게 들을 수 있게 합니다. 2026-07-02 all8 pseudo-gold `threshold=2`, quiet gate off 기준 match ratio는 53.1%라, 현재 channel attribution 실패는 모델만의 문제가 아니라 reference channel 검수도 필요한 상태입니다.
