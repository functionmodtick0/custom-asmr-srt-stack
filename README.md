...
내장 energy splitter는 `CASRT_QWEN_ENERGY_*` env로 내부 튜닝할 수 있지만 WebUI 옵션으로 노출하지 않습니다. `CASRT_QWEN_ENERGY_MAX_CHUNK_MS`는 긴 interval을 자르는 실험 옵션이며 현재 실데이터 평가에서는 기본값으로 켜지 않습니다. `CASRT_QWEN_ENERGY_CHUNK_CONTEXT_MS`는 max chunk가 설정된 실험에서만 audio context를 추가하고 core midpoint ownership으로 중복 결과를 거릅니다.
