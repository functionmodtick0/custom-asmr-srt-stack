...
class CohereAsrRuntime:
    ...
    def transcribe(self, request: dict[str, Any]) -> list[dict[str, Any]]:
        model_id = cohere_checked_model_path(require_string(request.get("model_id"), "model_id"), "model_id")
    ...
    def generate_result(
        ...
        language = cohere_language(source_language)
        if not language:
            raise ValueError(f"unsupported Cohere ASR source_language {source_language!r}")
        ...
        with torch.no_grad():
            output_ids = model.generate(**inputs, max_new_tokens=cohere_max_new_tokens(), do_sample=False)
        audio_chunk_index = inputs.get("audio_chunk_index")
        token_rows = output_ids.tolist()
        texts = processor.batch_decode(token_rows, skip_special_tokens=True)
        if audio_chunk_index is not None:
            texts = processor._reassemble_chunk_texts(  # noqa: SLF001 - public decode delegates to this helper.
                texts,
                audio_chunk_index,
                "" if language in {"ja", "zh"} else " ",
            )
        text = texts[0] if texts else ""
        return CohereAsrResult(text=text, start_ms=0, end_ms=duration_ms)
    ...
    def load_model(self, model_id: str) -> tuple[Any, Any]:
        ...
        log(f"loading local Cohere ASR model: {model_id}")
        import torch
        from transformers import CohereAsrForConditionalGeneration, CohereAsrProcessor

        kwargs = cohere_model_kwargs(torch)
        model = CohereAsrForConditionalGeneration.from_pretrained(model_id, **kwargs)
        processor = CohereAsrProcessor.from_pretrained(model_id, local_files_only=True, trust_remote_code=False)
        model.eval()
...
def cohere_model_kwargs(torch_module: Any) -> dict[str, Any]:
    dtype = torch_dtype(torch_module, os.environ.get("CASRT_COHERE_ASR_DTYPE", "bfloat16"))
    device_map = os.environ.get("CASRT_COHERE_ASR_DEVICE_MAP")
...
def cohere_checked_model_path(value: str, name: str) -> str:
    try:
        path = Path(value).expanduser().resolve(strict=True)
    except FileNotFoundError as error:
        raise ValueError(f"{name} must be an existing local Cohere ASR snapshot directory") from error
    if not path.is_dir():
        raise ValueError(f"{name} must be an existing local Cohere ASR snapshot directory")
...
def cohere_language(source_language: str) -> str | None:
    return SOURCE_LANGUAGE_TO_COHERE.get(source_language.strip().lower())


def cohere_max_new_tokens() -> int:
    return int(os.environ.get("CASRT_COHERE_ASR_MAX_NEW_TOKENS", "512"))
...
def wav_bytes_to_float_mono_16k(audio_bytes: bytes) -> np.ndarray:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        waveform, _ = librosa.load(path, sr=COHERE_SAMPLE_RATE, mono=True)
...
def torch_dtype(torch_module: Any, value: str) -> Any:
    ...
    raise ValueError("CASRT_COHERE_ASR_DTYPE must be bfloat16, float16, or float32")
...
def disable_python_network_if_requested() -> None:
    ...
    if _NETWORK_DISABLED:
        return
    if os.environ.get("CASRT_COHERE_ASR_DISABLE_NETWORK", "").strip().lower() not in {"1", "true", "yes"}:
        return
    ...
    class BlockedSocket(original_socket):  # type: ignore[misc, valid-type]
        def __new__(cls, *args: Any, **kwargs: Any) -> Any:
            del args, kwargs
            raise OSError("network access is disabled for local Cohere ASR worker")
...
def log(message: str) -> None:
    print(f"[casrt-cohere-asr-worker] {message}", file=sys.stderr, flush=True)


def handle_request(runtime: CohereAsrRuntime, request: dict[str, Any]) -> dict[str, Any]:
...
def response_for_line(runtime: CohereAsrRuntime, line: str) -> dict[str, Any]:
    try:
