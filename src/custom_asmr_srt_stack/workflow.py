...
def analyze_project(store: ProjectStore, project_id: str) -> dict[str, Any]:
    ...
    return store.save_audio_analysis(
        project_id,
        audio_info.to_json(),
        chunk_intervals(audio_info.duration_ms, max_chunk_ms=PRODUCTION_CHUNK_MS),
        channel_audio,
        normalized_wav,
    )
...
def transcribe_project(
    ...
    *,
    ...
    channel_names = transcription_channel_names(channels)
...
def transcription_chunks(
    ...
    adapter_limit_ms = adapter_max_chunk_ms(model_endpoint)
    max_chunk_ms = PRODUCTION_CHUNK_MS if adapter_limit_ms is None else min(PRODUCTION_CHUNK_MS, adapter_limit_ms)
...
def transcription_channel_names(channels: Any) -> list[str]:
