...
`model digest`는 local directory만 읽고 각 파일 SHA-256과 snapshot 전체 SHA-256을 기록한다. download/evaluation 결과 문서에는 exact revision path와 digest report path를 함께 적는다. 장기 보관할 model snapshot은 `/tmp`가 아니라 gitignored `.casrt/models/<model>-<revision>` 또는 persistent Hugging Face snapshot cache 아래에 둔다. digest report는 `.casrt/model-digests/<model>-<revision>-digest.json`에 둔다. `/tmp`는 재부팅이나 정리 작업으로 사라질 수 있으므로 다운로드 staging이나 삭제되어도 되는 실험 출력 전용이다.
...
- Qwen aligner worker는 segment text/channel을 바꾸지 않고 speech segment 내부 start/end만 재정렬한다. Stereo audio에서는 각 L/R/MIX segment와 같은 mono channel clip을 aligner에 넘기고, mono audio에 L/R label이 있으면 유일한 MIX 파형을 명시적으로 사용한다. `CASRT_QWEN_ALIGNER_MIN_ALIGNED_DURATION_MS`보다 짧은 span, 또는 원 segment duration 대비 `CASRT_QWEN_ALIGNER_MIN_COVERAGE_RATIO` 미만으로 줄어든 span은 원래 segment timing으로 유지한다. 기본값은 80ms와 0.5이며 WebUI 옵션으로 노출하지 않는다.
...
- Granite Plus local snapshot은 `.casrt/models/granite-speech-4.1-2b-plus-1454e6e1e33845ca9280ff65f52cf1141ba6e6e2`이고 digest report는 `.casrt/model-digests/granite-speech-4.1-2b-plus-1454e6e1e33845ca9280ff65f52cf1141ba6e6e2-digest.json`이다. snapshot SHA-256은 `1ef78d5809fbf87e6d2b7cad64aab4b7cb35a460683f988db09e83f216644326`다. Timestamp prompt 01/04/07 front120 평가에서는 practical CER 84.1%, Japanese relaxed CER 35.3%, time-aligned 500ms 22.2%, candidate MIX 53.7%, review effort 98.6%라 product gate를 실패했다. 기본 ASMR 경로로 승격하지 않는다.
...
- `vad coverage`와 `vad coverage-cases`는 `--channel-mode mix|stereo`를 지원하며 기본은 `mix`다. `stereo`는 실제 local stereo ASR 경로와 동일하게 L/R mono에서 VAD를 각각 실행하고 reference의 같은 channel 및 MIX speech만 coverage authority로 사용한다. Stereo suite의 `case_count`는 source case 수이고 `summary.coverage_unit_count`는 L/R unit 수다. 각 result는 `id=<case-id>:<channel>`, `case_id`, `channel`을 기록한다. Mono input, L/R로 분리할 수 없는 WAV, single-file `vad coverage --channel-mode stereo --intervals ...`는 fail-fast하며 WebUI 옵션은 추가하지 않는다.
...
- Root `item_count`와 `reason_counts`는 실제 packed item 기준으로 계산한다.
- Root `duration_summary`는 `source_item_duration_ms_sum`, `effective_item_duration_ms_sum`, `clip_duration_ms_sum`, `clip_duration_ms_max`, `focus_item_count`를 포함한다. 이 값은 focus clip이 review burden을 얼마나 줄였는지 반복 실험에서 비교하기 위한 관측 metric이며, item timing contract를 바꾸지 않는다. WebUI는 이 값을 review-pack header에 표시하지만 pack 생성 option은 추가하지 않는다.
