...
- `CASRT_QWEN_ASR_ALIGNER_MODEL_ID`는 고정 forced aligner 실험/내부 보정 경로다. `CASRT_QWEN_ASR_MIN_ALIGNED_DURATION_MS`보다 짧은 aligned span은 clip bounds로 되돌리며, WebUI 옵션으로 노출하지 않는다.
