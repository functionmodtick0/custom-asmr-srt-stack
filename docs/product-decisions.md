...
- 내부 VAD max-chunk 실험은 optional overlap context를 사용할 수 있다. Audio clip bounds와 gap/overlap 없이 검출 interval을 덮는 accept/core bounds를 분리하고, audio bounds는 file 범위 안에서 VAD/core 경계 바깥으로 확장한다. 모델 output의 절대시간 midpoint가 core에 속한 segment만 채택해 인접 context 중복을 제거한다. Context는 max chunk보다 작아야 하며 max chunk 없이 설정하거나 external VAD command와 결합하면 fail-fast한다. 이 설정은 평가 전용 env이고 WebUI 옵션으로 노출하지 않는다.
