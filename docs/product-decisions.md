...
로컬 ASR adapter인 `local-transformers`, `local-qwen-asr`, `local-cohere-asr`는 모두 MIX-first로 전사한다. L/R 단독 전사는 조용한 ASMR에서 bleed와 low-SNR 문제를 키우므로, L/R은 텍스트 입력이 아니라 channel attribution 근거로 사용한다.
...
- mono WAV는 `MIX`만 만든다.
- 모델 adapter는 `openai-compatible`, `gemini`, `local-transformers`, `local-qwen-asr`, `local-cohere-asr`다.
