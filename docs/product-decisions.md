...
- `Junlaii/Bro-ASR-1.7B`: Qwen3-ASR-1.7B-hf 기반 noisy Japanese full-merged safetensors 후보. Exact revision `6e8f651ad6137457a2a6e813b810349a7e272f4b`와 official Transformers exact commit runtime만 사용한다. 2026-08-30 external runtime 정적 보안 검토는 `PASS_WITH_CONSTRAINTS`이며, Qwen HF snapshot preflight, pinned chat template hash, offline/local-only/network block, persistent digest/runtime provenance를 충족한 뒤 official Qwen HF base와 같은 beam 5/all8 조건으로 비교한다. 실데이터 gate 전에는 기본 승격하지 않는다.
...
- `review-case-pack`: 준비된 case set의 reference `needs_review=true` segment만 잘라 기존 `custom-asmr-review-pack-v1` clip queue를 만든다. Root에는 `item_count`, `reason_counts`, `case_count`, `next_case_id`, `case_summaries`, `duration_summary`를 포함해 일반 `review-pack`과 같은 WebUI header/first-case navigation 계약을 제공한다. Human-reviewed 승격 전 사람이 남은 pseudo-gold 검수 구간만 빠르게 듣기 위한 CLI-only 보조 도구이며, 새 WebUI 옵션을 추가하지 않는다.
...
- `review-case-pack`은 `case-index.json`의 reference `needs_review=true` segment를 기존 review pack 형식으로 잘라 WebUI review-pack loader에서 들을 수 있게 한다. Root summary는 일반 review pack과 같은 header와 첫 검수 case navigation에 쓰이며, 이 산출물은 편의용 queue일 뿐 reference 편집/승격 source of truth는 여전히 case set이다.
