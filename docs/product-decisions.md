...
- `jaykwok/Qwen3-ASR-1.7B-JA-Anime-Galgame-hf`: 2026-08-30 community-first refresh의 current development-leading 후보. WhisperJAV v1.9의 실제 도메인 통합 기록은 base가 놓친 anime/galgame line을 복구하되 junk가 조금 늘 수 있다고 보고한다. Exact revision `5a6a789ceb2f22d2b8606743b13a8159af218362`의 native HF safetensors만 사용한다. Static external-surface review는 `PASS_WITH_CONSTRAINTS`; `.pt` CTC head와 conversion artifact는 받지 않는다. 01/04/08 사용자 실데이터 fixed L/R 20초 spot-audit에서 04/08은 Bro보다 도메인 어휘와 표현이 뚜렷하게 자연스러웠고, 01은 `介助`, `孤児棟` 같은 한자 오인이 늘었다. 세 case 모두 L/R bleed duplicate와 일부 후반 누락이 남았다. 사용자의 모델 자유 선택을 유지하므로 자동 default나 fallback으로 강제하지 않고 첫 수동 시도 후보로 문서화한다.
