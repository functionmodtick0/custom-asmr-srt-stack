...
- `jaykwok/Qwen3-ASR-1.7B-JA-Anime-Galgame-hf`: 2026-08-30 현재 제품 권장 모델로 확정한다. WhisperJAV v1.9의 실제 도메인 통합 기록은 base가 놓친 anime/galgame line을 복구하되 junk가 조금 늘 수 있다고 보고한다. Exact revision `5a6a789ceb2f22d2b8606743b13a8159af218362`의 native HF safetensors만 사용한다. Static external-surface review는 `PASS_WITH_CONSTRAINTS`; `.pt` CTC head와 conversion artifact는 받지 않는다. 01/04/08 사용자 실데이터 fixed L/R 20초 spot-audit에서 04/08의 도메인 어휘와 표현 개선이 컸고, 01은 `介助`, `孤児棟` 같은 한자 오인이 늘었다. 2/3 case의 질적 개선과 domain fit을 합쳐 이 모델을 Bro보다 우선한다고 판정한다. 세 case의 L/R bleed duplicate와 일부 후반 누락은 현재 권장 모델의 알려진 한계이며 사용자에게 추가 측정을 요구하지 않는다. 사용자의 자유 입력 결정을 지키기 위해 runtime에 강제하거나 fallback을 추가하지는 않는다.
...
2026-08-30 현재 권장 JA Anime-Galgame 모델의 01/04/08 fixed L/R 20초 출력에도 text/channel/segment를 보존하는 energy edge snap과 Qwen3-ForcedAligner를 다시 적용했다. Same-channel 500ms timing은 no-op `21.67%`, 가장 덜 해로운 energy edge snap `15.00%`, forced aligner `8.62%`, VAD+aligner `8.62%`였다. 따라서 VAD는 전사 삭제뿐 아니라 whole-chunk outer-bound 후처리로도 production에 넣지 않고, forced alignment도 no-op으로 유지한다. 이 결정은 VAD/alignment 일반론이 아니라 현재 20초 단일 transcript가 여러 발화를 합치는 data contract에서 얻은 결과다. 문장 단위 timestamp를 개선하려면 whole-segment 경계 이동이 아니라 audio 구간과 transcript substring의 ownership을 함께 증명하는 새 경로가 필요하다. Durable artifacts는 `.casrt/experiments/vad-edge-snap-ja-anime-20260830/`에 있다.
