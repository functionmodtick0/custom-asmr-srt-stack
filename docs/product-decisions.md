...
- `vad coverage`: built-in energy intervals 또는 precomputed VAD interval JSON을 reference speech union과 비교해 recall/precision/missed/extra duration을 계산한다. VAD/chunking 후보 비교용 CLI-only 진단 도구이며, ASR text 품질 gate나 WebUI 옵션으로 쓰지 않는다.
