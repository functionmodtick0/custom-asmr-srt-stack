...
- VAD coverage 진단의 channel mode는 실제 ASR channel mode와 일치해야 한다. `mix`는 기존 combined waveform 평가를 유지하고 `stereo`는 L/R audio와 same-channel+MIX reference를 각각 평가한다. Source case 수와 channel coverage unit 수는 별도 필드로 기록해 stereo를 case 두 배로 오해하지 않게 한다.
